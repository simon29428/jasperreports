...
public class FontUtil {
    ...
    private static final InheritableThreadLocal<Set<String>> threadMissingFontsCache =
        ...
        for (Iterator<FontFamily> itf = families.iterator(); itf.hasNext(); ) {
            ...
            if (locale == null || family.supportsLocale(locale)) {
                ...
                if (face != null) {
                    ...
                    } else if (awtFamilyMatchFontInfo == null
                            && face.getFont() != null
                    ...
                    if (fontFamily != null) {
                        ...
                        if (ignoreMissingFonts) {
                            if (log.isWarnEnabled()) {
                                log.warn("Font family " + fontSetFamily.getFamilyName() + " was not found for font set "
                                        + name);
                            }
                        } else {
     ...
     *
     */
    public Font getAwtFontFromBundles(
            boolean ignoreCase, String name, int style, float size, Locale locale, boolean ignoreMissingFont) {
        Font awtFont = null;
    ...
    protected Font getAwtFont(FontInfo fontInfo, int style, float size, boolean ignoreMissingFont) {
        ...
        if (face == null) {
            ...
            //			if (face == null)
            //			{
            //				throw new JRRuntimeException("Font family '" + family.getName() + "' does not have the normal font
            // face.");
            //			}
        } else {
        ...
        Font awtFont;
        if (face == null || face.getFont() == null) {
            // None of the family's font faces was found to match, neither by name, nor by style and the font family
            // does not even specify a normal face font.
            ...
            if (awtFont == null) {
                throw new JRRuntimeException(
                        EXCEPTION_MESSAGE_KEY_NULL_FONT, new Object[] {face.getName(), family.getName()});
            }
    ...
    public Font getAwtFontFromBundles(
            AwtFontAttribute fontAttribute, int style, float size, Locale locale, boolean ignoreMissingFont) {
    ...
    public Font getAwtFont(JRFont font, Locale locale) {
        ...
        Font awtFont = getAwtFontFromBundles(
                font.getFontName(),
    ...
    public Font resolveDeserializedFont(Font font) {
        // We use the font.getName() method here because the name field in the java.awt.Font class is the only font name
        // related information that gets serialized,
        // along with the size and style. The font.getFontName() and font.getFamily() both return runtime calculated
        // values, which are not accurate in case of AWT fonts
        // created at runtime through font extensions (both seem to return 'Dialog').
        // For AWT fonts created from font extensions using the Font.createFont(int, InputStream), the name field is set
        // to the same value as the font.getFontName(),
        // which is the recommended method to get the name of an AWT font.
        String fontName = font.getName();
        // We load an instance of an AWT font, even if the specified font name is not available
        // (ignoreMissingFont=true),
        // because only third-party visualization packages such as JFreeChart (chart themes) store serialized
        // java.awt.Font objects,
        // and they are responsible for the drawing as well.
        // Here we rely on the utility method ability to find a font by face name, not only family name. This is because
        // font.getName() above returns an AWT font name,
